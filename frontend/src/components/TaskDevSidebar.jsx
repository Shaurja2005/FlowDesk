      ...
      <div className="mb-3">
        ...
        >
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <span className="text-xs font-mono text-gray-300 truncate">Create Branch</span>
          </div>
          {copied ? <Check size={14} className="text-green-400" /> : <Copy size={14} className="text-gray-500 group-hover:text-primary-400" />}
        </button>
      </div>

      {/* Pull Requests */}
      <div>
        <p className="text-[10px] uppercase text-gray-500 font-bold mb-1 tracking-wider">Open PRs</p>
        {loading ? (
            ...
            {pulls.map(pr => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-xs text-gray-300 group-hover:text-white truncate">#{pr.number} {pr.title}</p>
                </div>
              </a>
            ))}
          </div>
